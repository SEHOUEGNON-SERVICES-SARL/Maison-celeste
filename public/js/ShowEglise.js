document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function fetchEglises(page) {
      try {
        const response = await fetch(`https://maison-celeste.onrender.com/api/alleglisesceleste?page=${page}&limit=${eglisesPerPage}`);
    ...
    function createEgliseCard(eglise) {
      ...
      image.src = `https://maison-celeste.onrender.com/api/firebase/${eglise.image}`;
    ...
    async function fetchEglisesCount() {
      try {
        const response = await fetch('https://maison-celeste.onrender.com/api/alleglisecleste/count');
        const data = await response.json();
        return data.count;
      } catch (error) {
        console.error('Erreur lors de la récupération du nombre total d\'églises:', error);
